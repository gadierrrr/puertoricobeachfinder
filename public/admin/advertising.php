...
/**
 * Parse a submitted datetime-local field.
 *
 * Returns null for a genuinely empty field ("no schedule") and false for input
 * that was present but unparseable. Callers must distinguish the two: they both
 * used to come back as null, which callers wrote straight into starts_at/ends_at,
 * so a malformed value silently converted a bounded paid window into a perpetual
 * one instead of being rejected.
 *
 * @return string|null|false
 */
function adAdminDateTime(string $key)
{
...
/**
 * Render a stored datetime for an <input type="datetime-local"> value.
 *
 * Browsers silently refuse to display a value that is not exactly
 * YYYY-MM-DDTHH:MM and submit the field empty instead. Migration 046 imports
 * legacy local_listings dates that are date-only ("2026-07-01"), so merely
 * opening such a campaign and pressing Save used to blank both dates. Padding
 * to midnight keeps the control populated and round-trips the value unchanged.
 */
function adAdminDateTimeLocalValue(?string $stored): string
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'record_incident') {
        ...
        if (!queryOne('SELECT id FROM ad_campaigns WHERE id=:id', [':id' => $campaignId])
            || $startedAt === null || $startedAt === false || $endedAt === false
