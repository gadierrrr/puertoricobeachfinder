<?php
/**
 * best-beaches-luquillo - SEO Landing Page
...
// Page metadata
$pageTitle = __('pages.best_beaches_luquillo.title');
$pageDescription = __('pages.best_beaches_luquillo.description');
$canonicalUrl = absoluteUrl('/best-beaches-luquillo');

$collectionKey = 'best-beaches-luquillo';
...
$pageFaqs = [];
for ($i = 1; $i <= 5; $i++) {
    $q = __("pages.best_beaches_luquillo.faq_{$i}_q");
    $a = __("pages.best_beaches_luquillo.faq_{$i}_a");
...
$breadcrumbs = [
    ...
    ['name' => __('pages.best_beaches_luquillo.breadcrumb')]
];
...
<section class="collection-content-nav bg-white border border-warm-200">
    <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex flex-wrap gap-2 justify-center text-sm">
            <span class="text-warm-500"><?= h(__('pages.best_beaches_luquillo.jump_to')) ?></span>
            <a href="#top-beaches" class="text-ocean-500 hover:underline"><?= h(__('pages.best_beaches_luquillo.jump_top_spots')) ?></a>
            <span class="text-warm-300">|</span>
            <a href="#tips" class="text-ocean-500 hover:underline"><?= h(__('pages.best_beaches_luquillo.jump_tips')) ?></a>
            <span class="text-warm-300">|</span>
            <a href="#faq" class="text-ocean-500 hover:underline"><?= h(__('pages.best_beaches_luquillo.jump_faq')) ?></a>
            <span class="text-warm-300">|</span>
            <a href="#map" class="text-ocean-500 hover:underline"><?= h(__('pages.best_beaches_luquillo.jump_map')) ?></a>
        </div>
    </div>
</section>
...
<section class="py-12">
    <div class="max-w-4xl mx-auto px-4">
        <div class="prose prose-lg max-w-none prose-brand">
            <p><?= __('pages.best_beaches_luquillo.intro_p1') ?></p>
            <p><?= __('pages.best_beaches_luquillo.intro_p2') ?></p>
        </div>
    </div>
</section>
...
<section id="tips" class="py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl md:text-3xl font-bold text-warm-900 mb-8 text-center">
            <?= h(__('pages.best_beaches_luquillo.tips_title')) ?>
        </h2>
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php for ($i = 1; $i <= 6; $i++): ?>
            <div class="bg-white border border-warm-200 rounded-xl p-6 shadow-card">
                <div class="text-3xl mb-4"><?= h(__("pages.best_beaches_luquillo.tip_{$i}_icon")) ?></div>
                <h3 class="text-lg font-bold text-warm-900 mb-2"><?= h(__("pages.best_beaches_luquillo.tip_{$i}_title")) ?></h3>
                <p class="text-warm-500 text-sm"><?= h(__("pages.best_beaches_luquillo.tip_{$i}_desc")) ?></p>
            </div>
            <?php endfor; ?>
        </div>
    </div>
...
<section id="faq" class="py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl md:text-3xl font-bold text-warm-900 mb-8 text-center">
            <?= h(__('pages.best_beaches_luquillo.faq_title')) ?>
        </h2>
        <div class="space-y-4">
...
<section id="map" class="py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl md:text-3xl font-bold text-warm-900 mb-8 text-center">
            <?= h(__('pages.best_beaches_luquillo.map_title')) ?>
        </h2>
        <div class="text-center">
            <a href="?view=map&collection=best-beaches-luquillo#top-beaches" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                <span><?= h(__('pages.best_beaches_luquillo.map_button')) ?></span>
            </a>
            <p class="text-warm-500 mt-4"><?= h(__('pages.best_beaches_luquillo.map_desc')) ?></p>
        </div>
    </div>
</section>
...
<section class="py-12 bg-emerald-600 text-white">
    <div class="max-w-4xl mx-auto px-4 text-center">
        <h2 class="text-2xl md:text-3xl font-bold mb-4"><?= h(__('pages.best_beaches_luquillo.cta_title')) ?></h2>
        <p class="text-lg opacity-90 mb-6"><?= h(__('pages.best_beaches_luquillo.cta_desc')) ?></p>
